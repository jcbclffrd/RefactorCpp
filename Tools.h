...
const int INT_INF =  10000000; //numeric_limits< int >::max() / 2;
...
// this method below allows for exprData(i,j), access of the data using the parenthesis.., note it doesn't return a pointer, rather the value of the pointer..
...
int  rand_site_index;	// from ~/C++exercises/Bins/Mscan/wtmx_scanmc1116.cpp
    rowLabels.clear();
    colLabels.clear();
    data.clear();
...
//for(int i; i < 5; i++){
    while ( !fin.eof() ) {
        ...
        vector< T > vals;
        while ( ss >> val ){ 
	 rand_site_index = gsl_ran_flat( rng, -1, 1 );
	 vals.push_back( val );
	} // while ss >> val
        data.push_back( vals );
    }
