...
{
    if (nRows <= 0 || nCols <= 0) {
        cerr << "Error: Invalid matrix dimensions: nRows = " << nRows << ", nCols = " << nCols << endl;
...
// *f is for example gsl_obj_f3,.. which requires a vector of parameters (denoted as gsl_vector*) and the ExprePredictor structure (denoted as void*).  The derivatives are local and hence require the point in space at which they're to be estimated, which is 'v'.  For example, 'v', holds ExprPar object.  The fourth input of numeric_deriv is 'params', which is simply the ExprPredictor structure (its object).  This is passed into *f inside the body of the code.  Hence *f, the function pointer, simply requires denoting the 'types' passed when denoting the function header for 'numeric_deriv'.  Evaluation of the function pointer is done inside of the body of the code, and the inputs to the actual function f (the obect pointed to by *f) requires passing arguments that are additional inputs to 'numeric_deriv (i.e. v and params; just as gsl_obj_f3 requires these two objects).  If we wanted to use numeric_deriv for a more complicated gsl_obj_f3 (say one that requires 3 inputs) then we would need to increase the number of arguments of numeric_deriv by one more input to denote the 3 inputs for *f (hence numeric_deriv would have 6 arguments instead of 5.
